...
antioquia <- antioquia %>%
  group_by(producto, depto_origen, anio) %>%
...
antioquia <- antioquia %>%
  group_by(depto_origen, mes_y_ano) %>%
...
antioquia <- antioquia %>%
  group_by(producto, depto_origen, mes_y_ano) %>%
...
antioquia <- antioquia %>%
  ...
  mutate(total_kilogramos_mes_destino_prod = sum(suma_kg))%>%ungroup()
...
antioquia<-antioquia%>%select(anio,depto_origen, mes,mpio_destino, producto,c(27:32))%>%
  filter(mpio_destino == "Medellín")%>%filter(depto_origen != "OTRO")

saveRDS(antioquia, "02_Indices/Output/base_indices_antioquia_cadena.rds")
